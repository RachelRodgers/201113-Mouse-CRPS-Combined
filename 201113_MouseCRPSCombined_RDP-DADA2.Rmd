---
title: "201113 Mouse CRPS Combined DADA2 Pathogen Server"
...
This data set contains both mouse and human samples.  Discard the human samples:

```{r sort-samples}
sampleData <- read.delim("../data/metadata/201113_Mouse_CRPS_combinedMetadata.txt",
                         check.names = FALSE, stringsAsFactors = FALSE)

mouseSamples <- sampleData %>% 
  pull(sample) %>% 
  as.character() %>% 
  paste0(".fastq")
...
inputFiles <- allInputFiles[allInputFiles %in% mouseSamples]
inputFilePath <- paste0("../data/raw_data/", inputFiles)
```
...
```{r eval=FALSE}
save.image("201113_Mouse_CRPS_Combined_dada2_Pathogen_Server.RData") # read back w/ load() or attach()
```
...
writeLines(capture.output(sessionInfo()), 
           "201113_MouseCRPSCombined_RDP-DADA2_Pathogen_Server_session_info.txt")
Sys.Date()
getwd()
sessionInfo()
```